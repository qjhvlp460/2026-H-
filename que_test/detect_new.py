...
# ============================================================
# 手动标定常量（★ 需现场测量，基于 ROI 裁剪后的坐标系 ★）
# 测量方法：运行后看 8081 端口通道0 画面，用看图工具读取像素坐标填入
# ============================================================
ROI_X0, ROI_Y0 = 170, 120   # 视野裁剪左上角（原图坐标）
ROI_X1, ROI_Y1 = 470, 360   # 视野裁剪右下角（原图坐标）

# --- 检测门控矩形（黄框）：所有识别只在该矩形内进行，框外噪点直接排除 ---
# 机械安装时让水管中心对准黄框中心
GATE_X0, GATE_Y0 = 15, 60     # TODO: 黄框左上角像素
GATE_X1, GATE_Y1 = 285, 150   # TODO: 黄框右下角像素

# --- 水管端线（用于像素→cm 比例 + 青色端线显示）---
PIPE_LEFT_X  = 25           # TODO: 水管左端面像素 x
PIPE_RIGHT_X = 275          # TODO: 水管右端面像素 x
PIPE_TOP_Y   = 85           # TODO: 水管上沿像素 y（青色端线绘制范围）
PIPE_BOT_Y   = 125          # TODO: 水管下沿像素 y（青色端线绘制范围）

PIPE_LENGTH_CM = 25.0                               # 水管实际长度（题目给定）
PIPE_CENTER_X  = (GATE_X0 + GATE_X1) / 2            # 中心点 O = 黄框中心（机械对准）
